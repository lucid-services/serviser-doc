...
function renderCurlExample(httpVerb, url, params) {
    let body;
    let headers =  _toList(params.header || {});

    if (_.isEmpty(params.body)) {
        body = '';
    } else {
        body = (JSON.stringify(params.body, null, 2) || '').trim();
    }

    if (headers.length) {
        _.last(headers).last = !!!body;
    }

    return render('code_snippets/curl', {
        httpVerb: httpVerb,
        url: url,
        hasHeadersOrBody: !(_.isEmpty(params.header) && _.isEmpty(params.body)),
        headers: headers,
        body: body,
        toUpperCase: _toUpperCase
    });
