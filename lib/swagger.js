...
var toSwagger = require('bi-ajv2swagger').toSwagger;
var _         = require('lodash');
var AMQPApp, ShellApp;
...
function buildSwaggerSpec(routers, app, options) {
    ...
    if (AMQPApp && app instanceof AMQPApp) {
        ...
        schemes.push('shell');
    } else if (app instanceof service.App) {
        schemes.push('https');
        schemes.push('http');
    }
    ...
    routers.forEach(function(router) {
        ...
        Object.keys(routes).forEach(function(url) {
            ...
            routes[url].forEach(function(route) {
                ...
                //debrecated
                //TODO remove sdkMethodName property (x-sdkMethodName should be used intead)
...
function describeRouteResponses(responses, validatorManager, route) {
    ...
    Object.keys(responses).forEach(function(code) {
        out[code] = responses[code].reduce(function(schema, descriptor) {
            ...
            //we've got ajv schema definition
            } else if (descriptor) {
                var validatorName
                ,   tmpValidatorName
                ,   validator;
