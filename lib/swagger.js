...
function buildSwaggerSpec(routers, app, options) {
    ...
    spec.info.description += buildMarkdownUrlSummary(urlList);

    return spec;
}

/**
 * @param {Array<Object>} urlList
 * @return {String}
 */
function buildMarkdownUrlSummary(urlList) {
    let out = '\n <details><summary> URL summary </summary>';

    urlList.forEach(function(item) {
        let method = pad(item.method.toUpperCase());
        out += `${method} [${item.url}](#operation/${item.uid})  \n`;
    });
