...
const toSwagger = require('serviser-ajv2swagger').toSwagger;
...
function generate(app, readme) {
    ...
    Object.keys(routers).forEach(function(version) {
        ...
        let port = app.config.get('doc:listen') || 80;
...
function buildSwaggerSpec(routers, app, options) {
    ...
    var spec = {
        openapi: '3.0.0',
        info: {
            title: app.config.get('doc:title') || 'serviser API documentation',
            'x-app': app.name,
            description: '',
    ...
    routers.forEach(function(router) {
        ...
        Object.keys(routes).forEach(function(url) {
            let relativeUrl = getRouteRelativeUrl(url, router);

            relativeUrl = relativeUrl.substr(options.basePath.length);
 ...
 * @return {String}
 */
function getRouteRelativeUrl(url, router) {
    let hostBasePath = router.App.config.get('basePath') || '';
    let relativeUrl;
    ...
    return router.$normalizeUrl(relativeUrl);
}
...
function generateCodeExamples(routeSchema, route) {
    ...
    let requestBody, contentType, sampleData;
    const out = [
        {
            lang: 'JavaScript',
            source: 'Whoops, something went wrong..'
        },
        {
            lang: 'Curl',
            source: 'Whoops, something went wrong..'
    ...
    try {
        sampleData = faker.generateRoutePrameters(
            routeSchema.parameters,
            requestBody
        );
    } catch(e) {
        //we do not want the service to abort its startup when x-code-samples
        //fails to be generated, so we are posponing the error event here
        route.Router.App.doc.once('listening', function() {
            this.emit('error', e);
        });
        return out;
    }
    ...
    out[0].source = renderer.renderJavaScriptExample(
        route.options.type,
        route.getAbsoluteUrl(sampleData.path),
        sampleData
    );

    out[1].source = renderer.renderCurlExample(
        route.options.type,
...
function describeRouteParameters(steps, validatorManager, route) {
    ...
    steps.forEach(function(step) {
        ...
        if (target === 'body' && out.requestBody !== undefined) {
            ...
            //TODO refactor after serviser-ajv2swagger fully supports OAS v3
...
function describeRouteResponses(responses, validatorManager, route) {
    //TODO refactor after serviser-ajv2swagger fully supports OAS v3
    //also requires major `serviser` update
    ...
    Object.keys(responses).forEach(function(code) {
        out[code] = responses[code].reduce(function(schema, descriptor) {
            ...
            //TODO refactor after serviser-ajv2swagger fully supports OAS v3
            //needs major update of serviser
...
function _getTagHeatmap(routers) {
    ...
    function parse(route) {
        let url = getRouteRelativeUrl(route.getUrl(), route.Router);
