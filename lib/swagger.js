...
function describeRouteResponses(responses) {
    ...
    Object.keys(responses).forEach(function(code) {
        ...
        //we've got json-inspector schema definition
            ...
            var inspector = new Validator(descriptor);
            out[code] = toSwagger(inspector)({in: 'body'})[0];
        }
    });
...
function getUrlTags(url, baseUrl) {
    ...
    if (urlParts.length >= 2) {
        //loop through all url parts except the last one
        for (var i = 0, len = urlParts.length -1; i < len; i++) {
            var tag = urlParts[i];
             if (!isBannedTag(tag)) {
                 tags.push(tag);
                 break;
        ...
        tags.push(urlParts.shift());
    } else {
        for (var y = baseUrlParts.length -1; y >= 0 ; y--) {
            var tag = baseUrlParts[y];
             if (!isBannedTag(tag)) {
                 tags.push(tag);
                 break;
             }
...
function isBannedTag(tag) {
    ...
    if (~banned.indexOf(tag) || semver.valid(tag) || tag.match('^\{[a-zA-Z0-9]+\}$')) {
        return true;
    }
