...
/**
 * @param {Object} specs
 * @param {Object} specs['<api_version>'] - OAS
 * @param {App} app
 * @return {Promise<Object>}
 */
function populateCodeSamples(specs, app) {
    Object.keys(specs).forEach(function(version) {
        Object.keys(specs[version].paths).forEach(function(url) {
            Object.keys(specs[version][url]).forEach(function(httpVerb) {
                const operationSchema = specs[version][url][httpVerb];
                const route = app.getRoute(operationSchema.operationId);
